...
export async function readCloudCatalog() {
  if (!isBlobConfigured()) return { products: PRODUCTS_DATA, source: 'default', blobs: [], storageConfigured: false };
...
export async function writeCloudCatalog(products) {
  ...
  if (previous.blobs.length) {
    await del(previous.blobs.map((blob) => blob.url)).catch(() => {});
  }
  return result;
}
...
export async function removeCloudImages(urls) {
  ...
  if (cloudUrls.length && isBlobConfigured()) await del(cloudUrls).catch(() => {});
}
